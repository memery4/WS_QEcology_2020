...
par(mfrow=c(1,1), mai=c(0.25,0.8,0.1, 0.1))
plot(Secchi_M_diff,typ="l", ylab="Secchi (meters)",xlab="Year");lines(fitted(arima_DOC),col="red")
```
```{r}
plot(Secchi_M$Date, Secchi_M_ts, typ="l",ylim=rev(range(Secchi_M_ts)), ylab="Secchi (meters)",xlab="Date")
```
...
Date_diff<-Secchi_M$Date[c(-1,0)]
par(mfrow=c(1,1), mai=c(0.25,0.8,0.1, 0.1))
plot(Secchi_M_diff,typ="l", ylab="Secchi (meters)",xlab="Year");lines(fitted(arima_TN),col="red")
```
```{r}
